...
if ( function_exists( 'acff' ) ) {
    ...
    if ( !function_exists( 'acff' ) ) {
        ...
        function acff()
        {
            ...
            // Turn off dev mode
            $acff->dev_mode = apply_filters( 'acfef_dev_mode', false );
            return $acff;
        }
    ...
    define( 'ACFEF_VERSION', '2.11.5' );
    define( 'ACFEF_ASSETS_VERSION', '6.9.15' );
    ...
    final class ACF_Frontend
    {
        ...
        public function init()
        {
            ...
            $this->acfef_notice_dismissed( 'pro_trial_dismiss' );
            add_filter(
                'plugin_row_meta',
                [ $this, 'acfef_row_meta' ],
                10,
        ...
        public function admin_notice_get_pro()
        {
            ...
            if ( in_array( 'administrator', (array) $user->roles ) ) {
				...
				<a href="' . add_query_arg( array(
                    'acfef_pro_trial_dismiss' => true,
                ), admin_url( 'admin.php?page=acfef-settings' ) ) . '"><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss notice.</span></button></a>
				</div>' ;
            }
        }
        
        public function acfef_notice_dismissed( $notice )
        {
            $user_id = get_current_user_id();
            if ( isset( $_GET['acfef_' . $notice] ) ) {
                add_user_meta(
                    $user_id,
                    'acfef_' . $notice,
                    'true',
