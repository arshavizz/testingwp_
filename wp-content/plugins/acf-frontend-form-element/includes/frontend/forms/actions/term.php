...
class ActionTerm extends ActionBase {
	...
	public function on_submit( $post_id, $form ){	
		...
		$term_name = '(no-name)';
		$term_key = 'acfef' . $wg_id . '_term_name';
		$slug_key = 'acfef' . $wg_id . '_term_slug';

		if( ! empty( $_POST['acf'][$term_key] ) ) {
			$term_name = acf_extract_var( $_POST['acf'], $term_key );	
		}

		$term_args = array();
		if( ! empty( $_POST['acf'][$slug_key] ) ) {
			$term_args['slug'] = acf_extract_var( $_POST['acf'], $slug_key );	
		}

			//$field = acf_get_field( $field_key );
			/* if( term_exists( $term_name, $_POST['_acf_taxonomy_type'] ) && ! empty( $field['change_slug'] ) ){
			} */


		$term_data = wp_insert_term( $term_name, $_POST['_acf_taxonomy_type'], $term_args );
