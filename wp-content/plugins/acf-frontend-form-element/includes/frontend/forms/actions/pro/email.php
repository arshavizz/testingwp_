...
class SendEmail extends ActionBase{
	...
	public function run( $post_id, $form, $step = false ){	
		$emails = $form['emails'];
		...
		if( $emails ){
			foreach( $emails as $email ){
				...
				foreach ( $fields as $key => $default ) {
					...
					$setting = acff()->dynamic_values->get_dynamic_values( $setting, $post_id, $form );
					if ( ! empty( $setting ) ) {
						$fields[ $key ] = $setting;
					}
				}
				 ...
				 */
				do_action( 'acf_frontend/mail_sent', $post_id, $email, $fields, $step );
			}
		}
