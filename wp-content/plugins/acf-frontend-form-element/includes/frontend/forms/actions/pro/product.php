	...
	public function get_fields_display( $form_field, $local_field, $wg_id = '', $sub_fields = false, $saving = false ){
		...
		switch( $form_field['field_type'] ){
			...
			case 'low_stock_threshold':
				$local_field['type'] = 'low_stock_threshold';			
			break;
			case 'stock_status':
...
/* 	public function create_product( $args ){	
	...
	public function on_submit( $post_id, $form ){	
		...
		if( $hook_name == 'add_product' ){
			...
			update_metadata( 'post', $post_id, 'acfef_form_source', $wg_id );
		}else{
			wp_update_post( $post_to_edit );
			update_metadata( 'post', $post_id, 'acfef_form_edited', $wg_id );
		}
		...
		//update product type
		$pt_field = acf_get_field( 'acfef_' . $wg_id . '_product_type' );
		$product_type = acf_extract_var( $_POST['acf'], 'acfef_' . $wg_id . '_product_type' );
		acf_update_value( $product_type, $post_id, $pt_field );
