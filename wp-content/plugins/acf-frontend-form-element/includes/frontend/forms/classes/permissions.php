...
class PermissionsTab{
    ...
    public function register_permissions_section( $widget, $step = false ){
        ...
        $widget->add_control( 'media_privacy_note', [
            ...
            'raw'             => __( '<p align="left">Click <a target="_blank" href="' . admin_url( '?page=acfef-settings&tab=uploads-privacy' ) . '">here</a> to limit the files displayed in the media library to the user who uploaded them.</p>', 'acf-frontend-form-element' ),
	...
	public function show_form( $settings ){
        ...
        if( isset( $settings['hidden_fields']['main_action'] ) ){
            $settings['main_action'] = $settings['hidden_fields']['main_action'];
        }
        ...
        if ( 'logged_in' == $settings['who_can_see'] ) {            
            if ( !is_user_logged_in() ) {
                ...
                if( isset( $settings['dynamic'] ) && ( $settings['main_action'] == 'edit_post' || $settings['main_action'] == 'delete_post' ) ) {
                    $post_id = get_the_ID();
	...
	public function __construct() {
		...
		add_filter( 'acfef/show_form', [ $this, 'show_form'], 10 );		
	}
