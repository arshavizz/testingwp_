...
class acf_field_term_name extends acf_field_text {
	...
	*/
    ...
    function validate_value( $is_valid, $value, $field, $input ){
        ...
        $term = $_POST['_acf_post_id'];
        $element_id = $_POST['_acf_element_id'];

        $slug_key = 'acfef_' . $element_id . '_term_slug';
		if( ! empty( $_POST['acf'][$slug_key] ) || ( empty( $field['change_slug'] ) && $term != 'add_term' ) ){
			return $is_valid;	
		}
        ...
        if( term_exists( $value, $_POST['_acf_taxonomy_type'] ) ){
            if( $term != 'add_term' ){
                $term_id = explode( 'term_', $term );
                if( ! empty( $term_id[1] ) ){
                    $term_to_edit = get_term( $term_id[1] );
                    if( $term_to_edit && $term_to_edit->slug == $value ){
                        return $is_valid;
                    }
                }
            }
